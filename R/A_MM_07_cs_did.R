...
c_turn  <- contrast_tbl %>% filter(outcome == "leader_turnover")
writeLines(
  c("% Auto-generated by A_MM_07_cs_did.R",
    ...
    # Archigos leader-turnover contrast (added as a second headline outcome).
    macro("MMCSContrastTurnover",
          if (nrow(c_turn)) safe_num(c_turn$att_diff, "%.4f") else "NA"),
    macro("MMCSContrastTurnoverSE",
          if (nrow(c_turn)) safe_num(c_turn$se_diff, "%.4f") else "NA"),
    macro("MMCSContrastTurnoverP",
          if (nrow(c_turn)) safe_num(c_turn$p_diff, "%.4f") else "NA"),
